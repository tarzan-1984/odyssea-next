...
/**
 * GET /api/bid-rates/by-chat/:chatRoomId/participants
 * Lists bid_rate_participants for +1 timers in BID chat.
 */
export async function GET(
	...
	try {
		...
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_BACKEND_URL}/v1/bid-rates/by-chat/${encodeURIComponent(chatRoomId)}/participants`,
			{
				method: "GET",
				headers: {
